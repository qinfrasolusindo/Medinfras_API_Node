...
import {
  ...
  BusinessLayerOperation,
} from "../business-layers/types";
...
async function runOperation(
  ...
  if (typeof method !== "function") {
    throw new AppError(
      500,
      `.NET method "${op.method}" was not found on BusinessLayer.`,
    );
  }
...
(async () => {
  const definitions = await businessLayers;
