...
app.post("/medinfras/mcp/messages", async (req: Request, res: Response) => {
  ...
  if (!transport) {
    res
      .status(400)
      .send(
        "No active SSE connection for this sessionId. Connect to GET /medinfras/mcp/sse first.",
      );
    return;
  }
...
app.listen(env.MCP_PORT, () => {
  console.log(
    `Medinfras MCP (SSE) server listening on http://localhost:${env.MCP_PORT}`,
  );

  console.log(
    `Connect MCP clients to:      http://localhost:${env.MCP_PORT}/medinfras/mcp/sse`,
  );
});
